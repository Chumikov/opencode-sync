...
Каждое устройство записывает в git **манифест** — список своих текущих сессий (`manifests/{device}.json`). При синхронизации:

- **Push** — экспортирует новые/изменённые сессии в JSON-файлы, обновляет манифест устройства, удаляет файлы сессий, которых нет ни в одном манифесте (все устройства удалили).
- **Pull** — подтягивает изменения из git, импортирует новые сессии, удаляет локальные сессии, которые были удалены на других устройствах (отсутствуют во всех манифестах).

Сессия удаляется с устройства когда **все устройства** синхронизировались без неё. Пока хотя бы одно устройство не обновило манифест — сессия считается живой.
...
- SQLite открывается в режиме READ-ONLY (кроме удаления сессий при pull)
